...
class StateMachine():
    ...
    def set_default_state_positions(self):
        ...
        if self.current_state == SystemStates.IGNITION:
            ...
            self.plc_workq.put(WorkQCmnd(WorkQCmnd_e.PLC_CLOSE_SOL, 6))
            self.plc_workq.put(WorkQCmnd(WorkQCmnd_e.PLC_OPEN_SOL, 7))
            self.plc_workq.put(WorkQCmnd(WorkQCmnd_e.PLC_OPEN_SOL, 8))
            self.plc_workq.put(WorkQCmnd(WorkQCmnd_e.PLC_CLOSE_SOL, 9))
            self.plc_workq.put(WorkQCmnd(WorkQCmnd_e.PLC_OPEN_SOL, 10))
            self.plc_workq.put(WorkQCmnd(WorkQCmnd_e.PLC_CLOSE_SOL, 11))
    ...
    def handle_valve_change(self, command: str, play_led_notif: threading.Event, seizer_mode: threading.Event) -> None:
        """
        ...
        if command in ["HEATER_ON", "HEATER_OFF", "PBV8_OPEN", "PBV8_CLOSE", "PUMP3_OFF", "PUMP3_ON", "SOL9_OPEN", "SOL9_CLOSE", "PBV10_OPEN", "PBV10_CLOSE", "PBV11_CLOSE"]:
        ...
        if command == "PBV1_OPEN":
            ...
            self.plc_workq.put(WorkQCmnd(WorkQCmnd_e.PLC_OPEN_SOL, 5))
        elif command == "SOL6_OPEN":
            self.plc_workq.put(WorkQCmnd(WorkQCmnd_e.PLC_OPEN_SOL, 6))
        elif command == "SOL7_OPEN":
            self.plc_workq.put(WorkQCmnd(WorkQCmnd_e.PLC_OPEN_SOL, 7))
        elif command == "SOL8_OPEN":
            self.plc_workq.put(WorkQCmnd(WorkQCmnd_e.PLC_OPEN_SOL, 8))
            ...
            self.plc_workq.put(WorkQCmnd(WorkQCmnd_e.PLC_CLOSE_SOL, 5))
        elif command == "SOL6_CLOSE":
            self.plc_workq.put(WorkQCmnd(WorkQCmnd_e.PLC_CLOSE_SOL, 6))
        elif command == "SOL7_CLOSE":
            self.plc_workq.put(WorkQCmnd(WorkQCmnd_e.PLC_CLOSE_SOL, 7))
...
def change_led_pattern(led_state: int, plc_workq: mp.Queue) -> None:
    ...
    if led_state == 0:
        ...
        plc_workq.put(WorkQCmnd(WorkQCmnd_e.PLC_CLOSE_PBV, 10))
    elif led_state == 4:
        plc_workq.put(WorkQCmnd(WorkQCmnd_e.PLC_CLOSE_SOL, 9))
        plc_workq.put(WorkQCmnd(WorkQCmnd_e.PLC_CLOSE_PBV, 8))
        plc_workq.put(WorkQCmnd(WorkQCmnd_e.PLC_HEATER_ON, None))
...
def state_thread(state_workq: mp.Queue, plc_workq: mp.Queue, database_workq: mp.Queue):
    ...
    play_led_notif = threading.Event()
    seizer_mode = threading.Event()
    auto_led_thread = threading.Thread(target=auto_led_pattern, args=(play_led_notif, seizer_mode, plc_workq))
    ...
    while 1:
        ...
        if message.command == WorkQCmnd_e.KILL_PROCESS:
            ...
            state_machine.handle_valve_change(message.data, play_led_notif, seizer_mode)
